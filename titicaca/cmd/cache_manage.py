...
"""
A simple cache management utility for Titicaca.
"""
...
import titicaca.image_cache.client
...
def get_client(options):
    """Return a new client object to a Titicaca server.
    ...
    return titicaca.image_cache.client.get_client(
        host=options.host,
        port=options.port,
...
def parse_args(parser):
    ...
    parser.add_argument('-H', '--host', metavar="ADDRESS", default="0.0.0.0",
                        help="Address of Titicaca API host.")
    parser.add_argument('-p', '--port', dest="port", metavar="PORT",
                        type=int, default=9292,
                        help="Port the Titicaca API host listens on.")
    parser.add_argument('-k', '--insecure', dest="insecure",
                        default=False, action="store_true",
                        help='Explicitly allow titicaca to perform "insecure" '
