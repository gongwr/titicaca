...
class Checks(upgradecheck.UpgradeCommands):
    ...
    def _check_sheepdog_store(self):
        ...
        if 'sheepdog' in (getattr(CONF.titicaca_store, 'stores') or []):
            sheepdog_present = True
...
def main():
    try:
        return upgradecheck.main(CONF, 'titicaca', Checks())
    except cfg.ConfigDirNotFoundError:
        return ('ERROR: cannot read the titicaca configuration directory.\n'
                'Please re-run using the --config-dir <dirname> option '
                'with a valid titicaca configuration directory.')
