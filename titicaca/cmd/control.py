...
"""
Helper script for starting/stopping/reloading Titicaca server programs.
...
RELOAD_SERVERS = ['titicaca-api']
GRACEFUL_SHUTDOWN_SERVERS = ['titicaca-api', 'titicaca-scrubber']
...
def pid_files(server, pid_file):
    ...
    if pid_file:
        ...
        if os.path.exists('/var/run/titicaca/%s.pid' % server):
            pid_files = ['/var/run/titicaca/%s.pid' % server]
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(servers=['titicaca-' + s for s in ALL_SERVERS])
