...
def main():
    ...
    try:
        ...
        app = scrubber.Scrubber(titicaca_store)
        ...
        if CONF.restore:
            ...
            if scrubber_already_running:
                already_running_msg = (
                    "ERROR: titicaca-scrubber is already running. "
...
def scrubber_already_running_posix():
    # Try to check the titicaca-scrubber is running or not.
    # 1. Try to find the pid file if scrubber is controlled by
    #    titicaca-control
    # 2. Try to check the process name.
    pid_file = '/var/run/titicaca/titicaca-scrubber.pid'
    if os.path.exists(os.path.abspath(pid_file)):
        return True

    for titicaca_scrubber_name in ['titicaca-scrubber',
                                 'titicaca.cmd.scrubber']:
        cmd = subprocess.Popen(
            ['/usr/bin/pgrep', '-f', titicaca_scrubber_name],
        ...
        if pids.count('\n') > 1 and str(self_pid) in pids:
            ...
            # means that another titicaca-scrubber process is running.
            return True
        elif pids.count('\n') > 0 and str(self_pid) not in pids:
            # If self is not in result and the pids number is still
            # > 0, it means that the another titicaca-scrubber process is
            # running.
            return True
