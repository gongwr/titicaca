...
import titicaca.db as db_api
from titicaca.i18n import _, _LC, _LE, _LI, _LW
...
Typically when an image is deleted, the ``titicaca-api`` service puts the image
into ``deleted`` state and deletes its data at the same time. Delayed delete
is a feature in Titicaca that delays the actual deletion of image data until a
later point in time (as determined by the configuration option ``scrub_time``).
When delayed delete is turned on, the ``titicaca-api`` service puts the image
...
'pending delete' feature is enabled in Titicaca. Please make sure the
titicaca-scrubber daemon is stopped before restoring the image to avoid image
data inconsistency.
