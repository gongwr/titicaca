...
from titicaca.common import format_inspector
...
class ImageRepoProxy(titicaca.domain.proxy.Repo):
    ...
    def __init__(self, image_repo, context, store_api, store_utils):
        ...
        self.db_api = titicaca.db.get_api()
...
def _get_member_repo_for_store(image, context, db_api, store_api):
    image_member_repo = titicaca.db.ImageMemberRepo(context, db_api, image)
    store_image_repo = titicaca.location.ImageMemberRepoProxy(
...
class ImageProxy(titicaca.domain.proxy.Image):
    ...
    def get_data(self, offset=0, chunk_size=None):
        if not self.image.locations:
            ...
            # https://bugs.launchpad.net/titicaca-store/+bug/1501443
            # When the above titicaca_store bug is fixed we can
            # add a msg as usual.
        ...
        for loc in self.image.locations:
            try:
                ...
                err = e
        # tried all locations
        LOG.error(_LE('Titicaca tried all active locations to get data for '
