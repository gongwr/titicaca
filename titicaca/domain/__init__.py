...
CONF = cfg.CONF
CONF.import_opt('task_executor', 'titicaca.common.config', group='task')
...
def _import_delayed_delete():
    # titicaca_store (indirectly) imports titicaca.domain therefore we can't put
    ...
    if not _delayed_delete_imported:
        CONF.import_opt('delayed_delete', 'titicaca_store')
...
class TaskExecutorFactory(object):
    ...
    def new_task_executor(self, context):
        try:
            ...
            executor_cls = ('titicaca.async_.%s_executor.'
