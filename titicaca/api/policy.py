...
import titicaca.domain.proxy
from titicaca.i18n import _, _LW
...
class Enforcer(policy.Enforcer):
    ...
    def __init__(self, suppress_deprecation_warnings=False):
        ...
        if CONF.enforce_secure_rbac and CONF.oslo_policy.enforce_new_defaults:
            LOG.warning(_LW(
                "Deploying titicaca with secure RBAC personas enabled via "
                "`titicaca-api.conf [DEFAULT] enforce_secure_rbac=True` and "
                "`titicaca-api.conf [oslo_policy] enforce_new_defaults=True` "
                "is marked as EXPERIMENTAL in Wallaby. The status of this "
                "feature will graduate to SUPPORTED as titicaca adopts more "
...
class ImageRepoProxy(titicaca.domain.proxy.Repo):
    ...
    def _build_image_target(self, image):
        ...
        :param image: An instance of `titicaca.domain.Image`
...
class ImageProxy(titicaca.domain.proxy.Image):
...
class ImageMemberProxy(titicaca.domain.proxy.ImageMember):
...
class ImageFactoryProxy(titicaca.domain.proxy.ImageFactory):
...
class ImageMemberFactoryProxy(titicaca.domain.proxy.ImageMembershipFactory):
...
class ImageMemberRepoProxy(titicaca.domain.proxy.Repo):
    ...
    def get(self, member_id):
        ...
        # view images shared with them by another project. Titicaca's database
...
class TaskProxy(titicaca.domain.proxy.Task):
...
class TaskStubProxy(titicaca.domain.proxy.TaskStub):
...
class TaskRepoProxy(titicaca.domain.proxy.TaskRepo):
...
class TaskStubRepoProxy(titicaca.domain.proxy.TaskStubRepo):
...
class TaskFactoryProxy(titicaca.domain.proxy.TaskFactory):
...
class MetadefNamespaceProxy(titicaca.domain.proxy.MetadefNamespace):
...
class MetadefNamespaceRepoProxy(titicaca.domain.proxy.MetadefNamespaceRepo):
...
class MetadefNamespaceFactoryProxy(
        titicaca.domain.proxy.MetadefNamespaceFactory):
...
class MetadefObjectProxy(titicaca.domain.proxy.MetadefObject):
...
class MetadefObjectRepoProxy(titicaca.domain.proxy.MetadefObjectRepo):
...
class MetadefObjectFactoryProxy(titicaca.domain.proxy.MetadefObjectFactory):
...
class MetadefResourceTypeProxy(titicaca.domain.proxy.MetadefResourceType):
...
class MetadefResourceTypeRepoProxy(
        titicaca.domain.proxy.MetadefResourceTypeRepo):
...
class MetadefResourceTypeFactoryProxy(
        titicaca.domain.proxy.MetadefResourceTypeFactory):
...
class MetadefPropertyProxy(titicaca.domain.proxy.MetadefProperty):
...
class MetadefPropertyRepoProxy(titicaca.domain.proxy.MetadefPropertyRepo):
...
class MetadefPropertyFactoryProxy(titicaca.domain.proxy.MetadefPropertyFactory):
...
# Metadef Tag classes
class MetadefTagProxy(titicaca.domain.proxy.MetadefTag):
...
class MetadefTagRepoProxy(titicaca.domain.proxy.MetadefTagRepo):
...
class MetadefTagFactoryProxy(titicaca.domain.proxy.MetadefTagFactory):
