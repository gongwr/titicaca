...
TITICACA_RESERVED_NS = 'os_titicaca'
_CACHED_THREAD_POOL = {}
...
def size_checked_iter(response, image_meta, expected_size, image_iter,
    ...
    if expected_size != bytes_written:
        ...
        LOG.error(msg)
        raise exception.TiticacaException(_("Corrupt image download for "
                                          "image %(image_id)s") %
                                        {'image_id': image_id})
...
def get_remaining_quota(context, db_api, image_id=None):
    ...
    Checks if it is allowed based on the given size in titicaca based on their
...
def get_thread_pool(lock_name, size=None):
    ...
    def _get_thread_pool():
        threadpool_cls = titicaca.async_.get_threadpool_model()
