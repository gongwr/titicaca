...
CONF = cfg.CONF
CONF.import_opt('public_endpoint', 'titicaca.api.versions')


class ImageDataController(object):
    def __init__(self, db_api=None, store_api=None,
                 policy_enforcer=None, notifier=None):
        db_api = db_api or titicaca.db.get_api()
        store_api = store_api or titicaca_store
        notifier = notifier or titicaca.notifier.Notifier()
        self.policy = policy_enforcer or titicaca.api.policy.Enforcer()
        self.gateway = titicaca.gateway.Gateway(db_api, store_api,
    ...
    def _unstage(self, image_repo, image, staging_store):
        ...
        if CONF.enabled_backends:
            file_path = "%s/%s" % (getattr(
                CONF, 'os_titicaca_staging_store').filesystem_store_datadir,
                image.image_id)
            try:
    ...
    def upload(self, req, image_id, data, size):
        ...
        if CONF.enabled_backends:
            backend = req.headers.get('x-image-meta-store',
                                      CONF.titicaca_store.default_backend)

            try:
                titicaca_store.get_store_from_store_identifier(backend)
        ...
        try:
            ...
            except (titicaca_store.NotFound,
        ...
        except titicaca_store.StoreAddDisabled:
        ...
        except titicaca_store.StorageFull as e:
        ...
        except titicaca_store.StorageWriteDenied as e:
    ...
    def stage(self, req, image_id, data, size):
        ...
        # titicaca_store is in a shape it is, the only way. Don't hold me
        # accountable for it.
        ...
        # NOTE(abhishekk): Use reserved 'os_titicaca_staging_store' for staging
        # the data, the else part will be removed once multiple backend feature
        # is declared as stable.
        if CONF.enabled_backends:
            staging_store = titicaca_store.get_store_from_store_identifier(
        ...
        try:
            ...
            try:
                ...
                image.size = size
            except titicaca_store.Duplicate:
            ...
            if self_url:
                image.extra_properties['os_titicaca_stage_host'] = self_url
...
class RequestDeserializer(wsgi.JSONRequestDeserializer):
    ...
    def stage(self, request):
        if "titicaca-direct" not in CONF.enabled_import_methods:
            msg = _("'titicaca-direct' method is not available at this site.")
...
class ResponseSerializer(wsgi.JSONResponseSerializer):
    ...
    def download(self, response, image):
        ...
        # titicaca/common/wsgi.py will raise HTTPRequestRangeNotSatisfiable
        ...
        except titicaca_store.NotFound as e:
            raise webob.exc.HTTPNoContent(explanation=e.msg)
        except titicaca_store.RemoteServiceUnavailable as e:
            raise webob.exc.HTTPServiceUnavailable(explanation=e.msg)
        except (titicaca_store.StoreGetNotSupported,
                titicaca_store.StoreRandomGetNotSupported) as e:
