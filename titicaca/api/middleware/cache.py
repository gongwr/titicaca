...
Titicaca API nodes. When images are requested from the API node,
...
from titicaca.api.common import size_checked_iter
...
class CacheFilter(wsgi.Middleware):
    ...
    def _get_v2_image_metadata(self, request, image_id):
        ...
        """
        db_api = titicaca.db.get_api()
        image_repo = titicaca.db.ImageRepo(request.context, db_api)
        try:
