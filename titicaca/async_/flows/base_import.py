...
class _ImportToFS(task.Task):
    ...
    def revert(self, image_id, result, **kwargs):
        ...
        if os.path.exists(result.split("file://")[-1]):
            if CONF.enabled_backends:
                store_api.delete(result, 'os_titicaca_tasks_store')
            else:
...
class _DeleteFromFS(task.Task):
    ...
    def execute(self, file_path):
        ...
        """
        if CONF.enabled_backends:
            store_api.delete(file_path, 'os_titicaca_tasks_store')
        else:
...
def _get_import_flows(**kwargs):
    ...
    extensions = named.NamedExtensionManager('titicaca.flows.import',
                                             names=['ovf_process',
                                                    'convert',
                                                    'introspect'],
                                             name_order=True,
...
def get_flow(**kwargs):
    ...
    :param image_factory: Titicaca Image Factory
