...
import titicaca.async_.flows._internal_plugins as internal_plugins
import titicaca.async_.flows.plugins as import_plugins
...
class _NoStoresSucceeded(exception.TiticacaException):
...
class ImportActionWrapper(object):
    ...
    def _assert_task_lock(self, image):
        task_lock = image.extra_properties.get('os_titicaca_import_task')
...
class _ImportActions(object):
    ...
    IMPORTING_STORES_KEY = 'os_titicaca_importing_to_stores'
    IMPORT_FAILED_KEY = 'os_titicaca_failed_import'
    ...
    def add_importing_stores(self, stores):
        ...
        Add stores to os_titicaca_importing_to_stores
    ...
    def remove_importing_stores(self, stores):
        ...
        Remove stores from os_titicaca_importing_to_stores
    ...
    def add_failed_stores(self, stores):
        ...
        Add stores to os_titicaca_failed_import
    ...
    def remove_failed_stores(self, stores):
        ...
        Remove stores from os_titicaca_failed_import
    ...
    def set_image_extra_properties(self, properties):
        ...
        for key, value in properties.items():
            if key.startswith(api_common.TITICACA_RESERVED_NS):
...
class _DeleteFromFS(task.Task):
    ...
    def execute(self, file_path):
        ...
        if CONF.enabled_backends:
            try:
                store_api.delete(file_path, 'os_titicaca_staging_store')
            ...
            # titicaca_store need to change this to use multi_backend
            # module.
...
class _ImportToStore(task.Task):
    ...
    def execute(self, file_path=None):
        ...
        #
        # if (base_path == CONF.titicaca_store.filesystem_store_datadir or
        #      base_path in CONF.titicaca_store.filesystem_store_datadirs):
...
def get_flow(**kwargs):
    ...
    if not uri and import_method in ['titicaca-direct', 'copy-image']:
        if CONF.enabled_backends:
