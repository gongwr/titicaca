...
class TaskExecutor(object):
    ...
    important Titicaca modules like, context, task_repo, image_repo,
    image_factory.
    ...
    Args:
        context: titicaca.context.RequestContext object for AuthZ and AuthN
            checks
        task_repo: titicaca.db.TaskRepo object which acts as a translator for
            titicaca.domain.Task and titicaca.domain.TaskStub objects
            into ORM semantics
        image_repo: titicaca.db.ImageRepo object which acts as a translator for
            titicaca.domain.Image object into ORM semantics
        image_factory: titicaca.domain.ImageFactory object to be used for
            creating new images for certain types of tasks viz. import, cloning
        admin_repo: titicaca.db.ImageRepo object which acts as a translator for
            titicaca.domain.Image object into ORM semantics, but with an admin
            context (optional)
    """
...
def get_threadpool_model():
    ...
    initialized, or if titicaca modules that use threading were imported
