...
from titicaca import assignment  # TODO(lbragstad): Decouple this dependency
from titicaca.common import cache
from titicaca.common import driver_hints
from titicaca.common import manager
from titicaca.common import provider_api
...
from titicaca.identity.mapping_backends import mapping
...
class DomainConfigs(provider_api.ProviderAPIMixin, dict):
    ...
    def _load_config_from_file(self, resource_api, file_list, domain_name):
        ...
        titicaca.conf.configure(conf=domain_config['cfg'])
        domain_config['cfg'](args=[], project='titicaca',
    ...
    def check_config_and_reload_domain_driver_if_required(self, domain_id):
        ...
        lightweight. More importantly, when the cache timeout is reached, we
        will get any config that has been updated from any other titicaca
        process.

        This cache-timeout approach works for both multiprocess and
        multithreaded titicaca configurations. In multithreaded
        ...
        process, next time it accesses the driver it will pick up the new one.
        ...
        if latest_domain_config:
            ...
            try:
                ...
                # multithreaded situation, two threads happen to be running
                # in lock step.
                pass
...
def domains_configured(f):
    ...
    to each call, and if it requires load them,
...
class Manager(manager.Manager):
    ...
    See :mod:`titicaca.common.manager.Manager` for more details on how this
    ...
    driver_namespace = 'titicaca.identity'
    ...
    def _require_matching_domain_id(self, new_ref, orig_ref):
        ...
        :param orig_ref: the dictionary of original values proposed for this
                         entity
        :raises: :class:`titicaca.exception.ValidationError`
        """
    ...
    def update_user(self, user_id, user_ref, initiator=None):
        ...
        if 'id' in user:
            ...
            # the driver layer won't be confused by the fact this is the
...
class MappingManager(manager.Manager):
    ...
    driver_namespace = 'titicaca.identity.id_mapping'
...
class ShadowUsersManager(manager.Manager):
    ...
    driver_namespace = 'titicaca.identity.shadow_users'
