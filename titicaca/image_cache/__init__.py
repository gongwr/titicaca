...
``titicaca.image_cache.drivers.base.Driver``. All image-cache drivers (existing
...
* The ``sqlite`` driver uses a sqlite database (which sits on every titicaca
...
Titicaca caches an image when it is downloaded. Consequently, the size of the
...
It is recommended to run cache-cleaner as a periodic task on the Titicaca API
...
Upon receiving the request to cache an image, Titicaca touches a file in the
        ...
        try:
            ...
            with self.driver.open_for_write(image_id) as cache_file:
                ...
                if (image_checksum and
                    ...
                    raise exception.TiticacaException(msg)
