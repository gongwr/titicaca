...
from titicaca.i18n import _, _LI


LOG = logging.getLogger(__name__)
CONF = cfg.CONF
CONF.import_opt('image_member_quota', 'titicaca.common.config')
CONF.import_opt('image_property_quota', 'titicaca.common.config')
CONF.import_opt('image_tag_quota', 'titicaca.common.config')
    ...
    def _enforce_image_property_quota(self, properties):
        ...
        attempted = len(
            [x for x in properties.keys()
             if not x.startswith(titicaca.api.common.TITICACA_RESERVED_NS)])
...
class QuotaImageLocationsProxy(object):
    ...
    def _check_user_storage_quota(self, locations):
        required_size = _calc_required_size(self.context,
                                            self.image,
                                            locations)
        titicaca.api.common.check_quota(self.context,
                                      required_size,
                                      self.db_api)
...
class ImageProxy(titicaca.domain.proxy.Image):
    ...
    def set_data(self, data, size=None, backend=None, set_active=True):
        remaining = titicaca.api.common.check_quota(
        ...
        #       - Now, to titicaca, image has not exceeded quota but, in
        #         reality, the quota has been exceeded.

        try:
            titicaca.api.common.check_quota(
