...
from titicaca.domain import proxy as domain_proxy
...
notifier_opts = [
    cfg.StrOpt('default_publisher_id',
               ...
               help=_("""
Default publisher_id for outgoing Titicaca notifications.

This is the value that the notification driver will use to identify
messages for events originating from the Titicaca service. Typically,
...
Possible values:
    ...
    * metadef_tag
    ...
    http://docs.openstack.org/developer/titicaca/notifications.html
...
def set_defaults(control_exchange='titicaca'):
...
def format_image_notification(image):
    """
    Given a titicaca.domain.Image object, return a dictionary of relevant
...
def format_image_member_notification(image_member):
    """Given a titicaca.domain.ImageMember object, return a dictionary of relevant
...
class ImageProxy(NotificationProxy, domain_proxy.Image):
    ...
    def _format_import_properties(self):
        ...
        failed = self.repo.extra_properties.get('os_titicaca_failed_import')
        failed = failed.split(',') if failed else []
        return {
            'os_titicaca_importing_to_stores': importing,
            'os_titicaca_failed_import': failed
        }
