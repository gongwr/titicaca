...
from titicaca.i18n import _, _LE, _LW
...
A property protection file, when set, restricts the Titicaca image
...
is not found, the titicaca-api service will fail to start.
More information on property protections can be found at:
https://docs.openstack.org/titicaca/latest/admin/property-protections.html
...
Provide the desired way to set property protection on Titicaca
...
https://docs.openstack.org/titicaca/latest/admin/property-protections.html#examples
...
class PropertyRules(object):
    ...
    def __init__(self, policy_enforcer=None):
        ...
        self.policies = []
        self.policy_enforcer = policy_enforcer or titicaca.api.policy.Enforcer()
    ...
    def _add_policy_rules(self, property_exp, action, rule):
        ...
        [prop_a]
        create = titicaca_creator
        then the corresponding policy rule would be:
        "prop_a:create": "rule:titicaca_creator"
        where titicaca_creator is defined in policy.yaml. For example:
        "titicaca_creator": "role:admin or role:titicaca_create_user"
        """
        rule = "rule:%s" % rule
