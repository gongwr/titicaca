...
def initialize_titicaca_store():
    """Initialize titicaca store."""
...
def initialize_multi_store():
    """Initialize titicaca multi store backends."""
...
class BaseServer(metaclass=abc.ABCMeta):
    ...
    This class requires initialize_titicaca_store set to True if
    titicaca store needs to be initialized.
    """
    def __init__(self, threads=1000, initialize_titicaca_store=False,
        ...
        self.running = True
        # NOTE(abhishek): Allows us to only re-initialize titicaca_store when
        # the API's configuration reloads.
        self.initialize_titicaca_store = initialize_titicaca_store
    ...
    def configure(self, old_conf=None, has_changed=None):
        ...
        if self.initialize_titicaca_store:
...
class PosixServer(BaseServer):
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            # NOTE(flaper87): When running titicaca-control,
            # (titicaca's functional tests, for example)
            # setpgid fails with EPERM as titicaca-control
            ...
            #
            # Running titicaca-(api|registry) is safe and
...
class Win32Server(BaseServer):
    ...
    def _get_sock_from_parent(self):
        ...
        if not pipe_handle:
            ...
            raise exception.TiticacaException(err_msg)
...
class Request(webob.Request):
    ...
    def best_match_language(self):
        ...
        if not self.accept_language:
            return None
        langs = i18n.get_available_languages('titicaca')
        ...
        if range_str is not None:
            ...
            if ',' in range_str:
                msg = ("Requests with multiple ranges are not supported in "
                       "Titicaca. You may make multiple single-range requests "
                       "instead.")
