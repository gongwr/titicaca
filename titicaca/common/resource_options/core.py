...
"""Options specific to resources managed by Titicaca (Domain, User, etc)."""

from titicaca.common import validation
...
class ResourceOption(object):
    ...
    def __init__(self, option_id, option_name, validator=_validator, json_schema_validation=None):
        """The base object to define the option(s) to be stored in the DB.

        :param option_id: The ID of the option. This will be used to look up
        ...
        if not isinstance(option_id, str) and len(option_id) == 4:
            raise TypeError(_('`option_id` must be a string, got %r') % option_id)
