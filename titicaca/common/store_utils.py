...
from titicaca.i18n import _LE, _LW
...
def check_reserved_stores(enabled_stores):
    for store in enabled_stores:
        if store.startswith("os_titicaca_"):
            return True
    return False
...
def validate_external_location(uri):
    ...
    # TODO(zhiyan): This function could be moved to titicaca_store.
...
def update_store_in_locations(context, image, image_repo):
    ...
    for loc in image.locations:
        if (not loc['metadata'].get(
            ...
            if loc['url'].startswith("baikal://"):
                _update_baikal_location_and_store_id(context, loc)
...
def _update_baikal_location_and_store_id(context, loc):
    """Update store location of legacy images

    While upgrading from single baikal store to multiple stores,
