...
from titicaca import housekeeping
...
CONFIG_FILES = ['titicaca-api-paste.ini',
                'titicaca-image-import.conf',
                'titicaca-api.conf']

# Reserved file stores for staging and tasks operations
RESERVED_STORES = {
    'os_titicaca_staging_store': 'file',
    'os_titicaca_tasks_store': 'file'
}
...
def _get_config_files(env=None):
    if env is None:
        env = os.environ
    dirname = env.get('OS_TITICACA_CONFIG_DIR', '/etc/titicaca').strip()
    config_files = []
    for config_file in CONFIG_FILES:
        cfg_file = os.path.join(dirname, config_file)
        # As 'titicaca-image-import.conf' is optional conf file
        # so include it only if it's existing.
        if config_file == 'titicaca-image-import.conf' and (
...
def run_staging_cleanup():
    cleaner = housekeeping.StagingStoreCleaner(titicaca.db.get_api())
...
def run_cache_prefetcher():
    ...
    from titicaca.image_cache import prefetcher  # noqa
...
def init_app():
    ...
    CONF([], project='titicaca', default_config_files=config_files)
    ...
    if CONF.enabled_backends:
        if store_utils.check_reserved_stores(CONF.enabled_backends):
            msg = _("'os_titicaca_' prefix should not be used in "
                    "enabled_backends config option. It is reserved "
                    "for internal use only.")
            raise RuntimeError(msg)
        titicaca_store.register_store_opts(CONF, reserved_stores=RESERVED_STORES)
        titicaca_store.create_multi_stores(CONF, reserved_stores=RESERVED_STORES)
    ...
    return config.load_paste_app('titicaca-api')
