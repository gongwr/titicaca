...
class TestScrubber(functional.FunctionalTest):
    ...
    def _get_pending_delete_image(self, image_id):
        # In Titicaca V2, there is no way to get the 'pending_delete' image from
    ...
    def test_scrubber_app(self):
        """
        test that the titicaca-scrubber script runs successfully when not in
        daemon mode
        """
        self.cleanup()
        kwargs = self.__dict__.copy()
        ...
        exe_cmd = "%s -m titicaca.cmd.scrubber" % sys.executable
        cmd = ("%s --config-file %s" %
    ...
    def test_scrubber_app_queue_errors_not_daemon(self):
        """
        test that the titicaca-scrubber exits with an exit code > 0 when it
    ...
    def test_scrubber_restore_image_with_daemon_running(self):
        ...
        exitcode, out, err = execute(cmd, raise_error=False)
        self.assertEqual(1, exitcode)
        self.assertIn('titicaca-scrubber is already running', str(err))
    ...
    def wait_for_scrubber_shutdown(self, func):
        ...
        # Sometimes the titicaca-scrubber process which is setup by the
        # previous test can't be shutdown immediately, so if we get the "daemon
        # running" message we sleep and try again.
        not_down_msg = 'titicaca-scrubber is already running'
        total_wait = 15
