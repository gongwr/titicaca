...
class BaseServer(metaclass=abc.ABCMeta):
    ...
    def write_conf(self, **kwargs):
        ...
        # to trample on currently-running Titicaca servers, now do we?
    ...
    def create_database(self):
        ...
        if self.needs_database:
            conf_dir = os.path.join(self.test_dir, 'etc')
            utils.safe_mkdirs(conf_dir)
            conf_filepath = os.path.join(conf_dir, 'titicaca-manage.conf')
            ...
            if titicaca_db_env in os.environ:
                ...
                cmd = ('%s -m titicaca.cmd.manage --config-file %s db sync' %
    ...
    def start(self, expect_exit=True, expected_exitcode=0, **kwargs):
        ...
        if self.sock:
            if not self.fork_socket:
                ...
                exec_env[utils.TITICACA_TEST_SOCKET_FD_STR] = str(fd)
                pass_fds.add(fd)
                self.sock.close()
...
class ApiServer(Server):
    ...
    def __init__(self, test_dir, port, policy_file, delayed_delete=False,
        ...
        self.server_module = 'titicaca.cmd.%s' % self.server_name
        ...
        secure_rbac = bool(os.getenv('OS_TITICACA_TEST_RBAC_DEFAULTS'))
        ...
        self.sql_connection = os.environ.get('TITICACA_TEST_SQL_CONNECTION',
...
allowed_ports = []
"""
        self.paste_conf_base = """[pipeline:titicaca-api]
pipeline =
    cors
    healthcheck
    versionnegotiation
...
class FunctionalTest(test_utils.BaseTestCase):
    ...
    def start_servers(self, **kwargs):
        """
        Starts the API and Registry servers (titicaca-control api start
        ) on unused ports.  titicaca-control
    ...
    def wait_for_servers(self, servers, expect_launch=True, timeout=30):
        ...
        :param servers: Titicaca server ports to ping
    ...
    def stop_server(self, server):
        ...
        titicaca-control stop method to gracefully shut the server down.
    ...
    def stop_servers(self):
        ...
        the titicaca-control stop method to gracefully shut the server down.
    ...
    def copy_data_file(self, file_name, dst_dir):
        src_file_name = os.path.join('titicaca/tests/etc', file_name)
...
class SynchronousAPIBase(test_utils.BaseTestCase):
    ...
    def setup_simple_paste(self):
        ...
        """
        self.paste_config = os.path.join(self.test_dir, 'titicaca-api-paste.ini')
        with open(self.paste_config, 'w') as f:
            f.write(textwrap.dedent("""
            [filter:context]
            paste.filter_factory = titicaca.api.middleware.context:\
                ContextMiddleware.factory
            [filter:fakeauth]
            paste.filter_factory = titicaca.tests.utils:\
                FakeAuthMiddleware.factory
            [filter:cache]
            paste.filter_factory = titicaca.api.middleware.cache:\
            CacheFilter.factory
            [filter:cachemanage]
            paste.filter_factory = titicaca.api.middleware.cache_manage:\
    ...
    def setup_stores(self):
        ...
        store2, and store3) as well as a os_titicaca_staging_store for
        imports.
        ...
        self.config(filesystem_store_datadir=self._store_dir('staging'),
                    group='os_titicaca_staging_store')
        self.config(filesystem_store_datadir=self._store_dir('tasks'),
                    group='os_titicaca_tasks_store')
    ...
    def start_server(self, enable_cache=False):
        ...
        root_app = 'titicaca-api'
        if enable_cache:
            root_app = 'titicaca-api-cachemanagement'
    ...
    def _import_direct(self, image_id, stores, headers=None):
        """Do an import of image_id to the given stores."""
        body = {'method': {'name': 'titicaca-direct'},
                'stores': stores,
                'all_stores': False}
    ...
    def _wait_for_import(self, image_id, retries=10):
        for i in range(0, retries):
            ...
            if not image.get('os_titicaca_import_task'):
                break
            self.addDetail('Create-Import task id',
                           ttc.text_content(image['os_titicaca_import_task']))
            time.sleep(1)

        self.assertIsNone(image.get('os_titicaca_import_task'),
