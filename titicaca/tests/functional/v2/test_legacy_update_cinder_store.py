...
from titicaca_store._drivers import cinder
...
class TestLegacyUpdateCinderStore(functional.SynchronousAPIBase):
    ...
    def setup_single_store(self):
        ...
        self.config(default_store='cinder', group='titicaca_store')
        self.config(stores=['http', 'swift', 'cinder'], group='titicaca_store')
        self.config(cinder_volume_type='fast', group='titicaca_store')
        self.config(cinder_store_user_name='fake_user', group='titicaca_store')
        self.config(cinder_store_password='fake_pass', group='titicaca_store')
    ...
    def unset_single_store(self):
        ...
        self.config(cinder_volume_type='', group='titicaca_store')
        self.config(cinder_store_user_name='', group='titicaca_store')
        self.config(cinder_store_password='', group='titicaca_store')
        self.config(cinder_store_project_name='', group='titicaca_store')
        self.config(cinder_store_auth_address='', group='titicaca_store')
        titicaca_store.create_stores(CONF)
    ...
    def setup_multiple_stores(self, mock_client):
        ...
        store2) as well as a os_titicaca_staging_store for
        imports.
    ...
    @mock.patch('titicaca_store._drivers.cinder.Store._wait_volume_status')
    ...
    def test_create_image(self, mock_host_addr, mock_mask_pass, mock_wait,
        ...
        # NOTE(whoami-rajat): These are internals called by titicaca_store, so
