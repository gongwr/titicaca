...
class TestImagesPolicy(functional.SynchronousAPIBase):
    ...
    @mock.patch('titicaca.location._check_image_location', new=lambda *a: 0)
    @mock.patch('titicaca.location.ImageRepoProxy._set_acls', new=lambda *a: 0)
    ...
    def test_import_titicaca_direct(self):
        self.start_server()
        # create image and stage data to it
        image_id = self._create_and_stage(visibility='public')

        # Make sure you can import using titicaca-direct
        self.set_policy_rules({
            'get_image': '',
            'communitize_image': '',
            'add_image': '',
