...
class TestImageImportLocking(functional.SynchronousAPIBase):
    def _get_image_import_task(self, image_id, task_id=None):
        if task_id is None:
            ...
            task_id = image['os_titicaca_import_task']
    ...
    def _test_import_copy(self, warp_time=False):
        ...
        with mock.patch('titicaca.location.ImageProxy.set_data') as mock_sd:
    ...
    def test_import_copy_bust_lock(self):
        ...
        self.assertEqual('', image['os_titicaca_failed_import'])
        ...
        image = self.api_get('/v2/images/%s' % image_id).json
        self.assertEqual('', image.get('os_titicaca_import_task', ''))
        self.assertEqual('', image['os_titicaca_importing_to_stores'])
        self.assertEqual('', image['os_titicaca_failed_import'])
        self.assertEqual('store1,store3', image['stores'])
    ...
    def test_import_task_status(self):
        ...
        # "upload this to titicaca_store" part, which we override so we
        ...
        with mock.patch('titicaca.location.ImageProxy._upload_to_store') as mu:
