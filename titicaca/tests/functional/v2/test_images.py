...
from titicaca.tests.functional import ft_utils as func_utils
...
class TestImages(functional.FunctionalTest):
    ...
    def test_image_import_using_titicaca_direct(self):
        ...
        # titicaca-direct should be available in discovery response
        path = self._url('/v2/info/import')
        response = requests.get(path, headers=self._headers())
        self.assertEqual(http.OK, response.status_code)
        discovery_calls = jsonutils.loads(
            response.text)['import-methods']['value']
        self.assertIn("titicaca-direct", discovery_calls)
    ...
    def test_image_lifecycle(self):
        ...
        # The "changes-since" filter shouldn't work on titicaca v2
    ...
    def _create_qcow(self, size):
        fn = tempfile.mktemp(prefix='titicaca-unittest-images-',
                             suffix='.qcow2')
    ...
    def test_download_random_access_w_content_range(self):
        ...
        for backward compatibility with clients written for pre-Pike Titicaca.
    ...
    def test_property_protections_with_policies(self):
        # Enable property protection
        rules = {
            "titicaca_creator": "role:admin or role:spl_role"
        }
        self.set_policy_rules(rules)
...
class TestImagesMultipleBackend(functional.MultipleBackendFunctionalTest):
    ...
    def test_image_import_using_titicaca_direct(self):
        ...
        # os_titicaca_staging_store should not be available in discovery response
        for stores in discovery_calls:
            self.assertIn('id', stores)
            self.assertIn(stores['id'], available_stores)
            self.assertFalse(stores["id"].startswith("os_titicaca_"))
    ...
    def test_image_import_using_titicaca_direct_different_backend(self):
        self.start_servers(**self.__dict__.copy())
        ...
        # Upload some image data to staging area
        image_data = b'TITICACA IS DEAD SEXY'
        path = self._url('/v2/images/%s/stage' % image_id)
        headers = self._headers({'Content-Type': 'application/octet-stream'})
        response = requests.put(path, headers=headers, data=image_data)
        self.assertEqual(http.NO_CONTENT, response.status_code)
    ...
    def test_copy_image_revert_lifecycle(self):
        ...
        def poll_callback(image):
            ...
            return not (image['os_titicaca_importing_to_stores'] == '' and
                        image['os_titicaca_failed_import'] == 'file3' and
                        image['stores'] == 'file1')
    ...
    def test_image_lifecycle_different_backend(self):
        ...
        # Upload some image data
        image_data = b'just a passing titicaca'
        path = self._url('/v2/images/%s/file' % image_id)
        headers = self._headers({
...
class TestMultiStoreImageMembers(functional.MultipleBackendFunctionalTest):
    ...
    def test_image_member_lifecycle_for_multiple_stores(self):
        ...
        try:
            ...
            # environment refer,
            # https://bugs.launchpad.net/titicaca/+bug/1873735
...
class TestImportProxy(functional.SynchronousAPIBase):
    ...
    def setUp(self):
        ...
        self.ksa_client = self.useFixture(
            fixtures.MockPatch('titicaca.context.get_ksa_client')).mock
    ...
    def test_import_proxy(self):
        ...
        self.assertNotIn('os_titicaca_stage_host', image)
