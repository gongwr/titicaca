...
from titicaca.db.sqlalchemy import models as db_models
from titicaca.db.sqlalchemy import models_metadef as metadef_models
import titicaca.tests.functional.db as db_tests
from titicaca.tests.functional.db import base
from titicaca.tests.functional.db import base_metadef
...
def get_db(config):
    ...
    config(debug=False)
    db_api = titicaca.db.sqlalchemy.api
    return db_api
...
class TestImageStorageUsage(base.TestDriver,
    ...
    def setUp(self):
        ...
        for owner in (uuids.owner1, uuids.owner2):
            ctxt = titicaca_context.RequestContext(project_id=owner)
