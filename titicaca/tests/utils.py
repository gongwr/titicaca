...
class BaseTestCase(testtools.TestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(titicaca_fixtures.WarningsFixture())

        # Make sure logging output is limited but still test debug formatting
        self.useFixture(log_fixture.get_logging_handle_error_fixture())
        self.useFixture(titicaca_fixtures.StandardLogging())
...
def fork_exec(cmd,
    ...
    example: titicaca-api.  When titicaca-api starts it sets itself as
...
def minimal_add_command(port, name, suffix='', public=True):
    ...
    return ("bin/titicaca --port=%d add %s"
