...
import titicaca.api.middleware.cache
...
from titicaca.tests.unit import test_policy
...
class TestCacheMiddlewareURLMatching(testtools.TestCase):
    def test_v2_match_id(self):
        ...
        out = titicaca.api.middleware.cache.CacheFilter._match_request(req)
...
class TestCacheMiddlewareRequestStashCacheInfo(testtools.TestCase):
    def setUp(self):
        ...
        self.middleware = titicaca.api.middleware.cache.CacheFilter
...
class ChecksumTestCacheFilter(titicaca.api.middleware.cache.CacheFilter):
    def __init__(self):
...
class ProcessRequestTestCacheFilter(titicaca.api.middleware.cache.CacheFilter):
    def __init__(self):
...
class TestCacheMiddlewareProcessRequest(base.IsolatedUnitTest):
    ...
    def test_verify_metadata_is_image_target_instance_with_zero_size(self):
        ...
        image.size = 0
        image_meta = titicaca.api.policy.ImageTarget(image)
        self._test_verify_metadata_zero_size(image_meta)
    ...
    def test_v2_process_request_download_restricted(self):
        ...
        def fake_get_v2_image_metadata(*args, **kwargs):
            ...
            request.environ['api.cache.image'] = image
            return image, titicaca.api.policy.ImageTarget(image)
    ...
    def test_v2_process_request_download_permitted(self):
        ...
        cache_filter.policy = titicaca.api.policy.Enforcer(
            suppress_deprecation_warnings=True)
