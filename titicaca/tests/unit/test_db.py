...
import titicaca.db
from titicaca.db.sqlalchemy import api
...
class TestImageRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.image_repo = titicaca.db.ImageRepo(self.context, self.db)
    ...
    def test_list_shared_images_other_tenant(self):
        ...
        context = titicaca.context.RequestContext(user=USER1, tenant=TENANT3)
        image_repo = titicaca.db.ImageRepo(context, self.db)
    ...
    def test_save_excludes_atomic_props(self):
        ...
        image.extra_properties['os_titicaca_import_task'] = fake_uuid
        self.image_repo.save(image)

        # Expect it was ignored
        image = self.image_repo.get(UUID1)
        self.assertNotIn('os_titicaca_import_task', image.extra_properties)

        # Set the property atomically
        self.image_repo.set_property_atomic(image,
                                            'os_titicaca_import_task', fake_uuid)
        # Expect it is set
        image = self.image_repo.get(UUID1)
        self.assertEqual(fake_uuid,
                         image.extra_properties['os_titicaca_import_task'])

        # Try to clobber it
        image.extra_properties['os_titicaca_import_task'] = 'foo'
        self.image_repo.save(image)
        ...
        # Try to delete it
        del image.extra_properties['os_titicaca_import_task']
        self.image_repo.save(image)
...
class TestImageMemberRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = titicaca.context.RequestContext(
            user=USER1, tenant=TENANT1)
        self.image_repo = titicaca.db.ImageRepo(self.context, self.db)
        self.image_member_factory = titicaca.domain.ImageMemberFactory()
        self._create_images()
        self._create_image_members()
        image = self.image_repo.get(UUID1)
        self.image_member_repo = titicaca.db.ImageMemberRepo(self.context,
                                                           self.db, image)
    ...
    def test_list_no_members(self):
        image = self.image_repo.get(UUID2)
        self.image_member_repo_uuid2 = titicaca.db.ImageMemberRepo(
    ...
    def test_remove_image_member_does_not_exist(self):
        fake_uuid = str(uuid.uuid4())
        image = self.image_repo.get(UUID2)
        fake_member = titicaca.domain.ImageMemberFactory().new_image_member(
            image, TENANT4)
...
class TestTaskRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.db = unit_test_utils.FakeDB(initialize=False)
        self.context = titicaca.context.RequestContext(user=USER1,
                                                     tenant=TENANT1)
        self.task_repo = titicaca.db.TaskRepo(self.context, self.db)
        self.task_factory = titicaca.domain.TaskFactory()
