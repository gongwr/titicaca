...
from titicaca.tests.unit import base as unit_test_base
...
class FakeMemberRepo(object):
    def __init__(self, image, tenants=None):
        self.image = image
        self.factory = titicaca.domain.ImageMemberFactory()
...
class TestStoreMultiBackends(utils.BaseTestCase):
    ...
    @mock.patch("titicaca.location.signature_utils.get_verifier")
    def test_set_data_calls_upload_to_store(self, msig):
        context = titicaca.context.RequestContext(user=USER1)
        extra_properties = {
        ...
        image = titicaca.location.ImageProxy(image_stub, context,
    ...
    @mock.patch('titicaca.location.LOG')
...
class TestStoreImage(utils.BaseTestCase):
    ...
    def test_image_delete(self):
        image = titicaca.location.ImageProxy(self.image_stub, {},
                                           self.store_api, self.store_utils)
        ...
        self.assertRaises(titicaca_store.NotFound,
    ...
    def test_image_get_data_from_second_location(self):
        ...
        image1 = titicaca.location.ImageProxy(self.image_stub, {},
                                            self.store_api, self.store_utils)
        ...
        with mock.patch("titicaca.location.store") as mock_store:
    ...
    @mock.patch('titicaca.common.format_inspector.get_inspector')
    ...
    def test_image_change_append_invalid_location_metatdata(self):
        ...
        self.assertRaises(titicaca_store.BackendException,
                          image1.locations.append, location_bad)
    ...
    def test_image_change_adding_invalid_location_uri(self):
        ...
        image1 = titicaca.location.ImageProxy(image_stub1, context,
                                            self.store_api, self.store_utils)
    ...
    def test_image_change_adding_invalid_location_metadata(self):
        ...
        image2 = titicaca.location.ImageProxy(image_stub2, context,
                                            self.store_api, self.store_utils)
    ...
    def test_image_change_adding_locations(self):
        ...
        image3 = titicaca.location.ImageProxy(image_stub3, context,
                                            self.store_api, self.store_utils)
...
class TestStoreImageRepo(utils.BaseTestCase):
    def setUp(self):
        ...
        self.image = titicaca.location.ImageProxy(self.image_stub, {},
                                                self.store_api, store_utils)
        self.image_repo_stub = ImageRepoStub()
        self.image_repo = titicaca.location.ImageRepoProxy(self.image_repo_stub,
                                                         {}, self.store_api,
                                                         store_utils)
        patcher = mock.patch("titicaca.location._get_member_repo_for_store",
                             self.get_fake_member_repo)
        patcher.start()
        self.addCleanup(patcher.stop)
        self.fake_member_repo = FakeMemberRepo(self.image, [TENANT1, TENANT2])
        self.image_member_repo = titicaca.location.ImageMemberRepoProxy(
    ...
    def test_member_addition_updates_acls(self):
        self.image_stub.locations = [{'url': 'glug', 'metadata': {},
                                      'status': 'active'}]
        self.image_stub.visibility = 'private'
        membership = titicaca.domain.ImageMembership(
...
class TestImageFactory(unit_test_base.StoreClearingUnitTest):
    ...
    def setUp(self):
        ...
        store_utils = unit_test_utils.FakeStoreUtils(store_api)
        self.image_factory = titicaca.location.ImageFactoryProxy(
            ImageFactoryStub(),
            titicaca.context.RequestContext(user=USER1),
            store_api,
            store_utils)
...
class TestStoreMetaDataChecker(utils.BaseTestCase):
    ...
    def test_empty(self):
        titicaca_store.check_location_metadata({})

    def test_unicode(self):
        m = {'key': u'somevalue'}
        titicaca_store.check_location_metadata(m)
