...
from titicaca.cmd import replicator as titicaca_replicator
from titicaca.common import exception
from titicaca.tests.unit import utils as unit_test_utils
...
class ImageServiceTestCase(test_utils.BaseTestCase):
    def test_rest_errors(self):
        c = titicaca_replicator.ImageService(FakeHTTPConnection(), 'noauth')
    ...
    def test_rest_header_list_to_dict(self):
        ...
        o = titicaca_replicator.ImageService._header_list_to_dict(i)
    ...
    def test_rest_dict_to_headers(self):
        ...
        o = titicaca_replicator.ImageService._dict_to_headers(i)
    ...
    def test_rest_add_image_meta(self):
        ...
        image_meta_headers = titicaca_replicator.ImageService._dict_to_headers(
            image_meta)
...
def check_no_args(command, args):
    ...
    orig_img_service = titicaca_replicator.get_image_service
    try:
        titicaca_replicator.get_image_service = get_image_service
        command(options, args)
    except TypeError as e:
        if str(e) == "Too few arguments.":
            no_args_error = True
    finally:
        titicaca_replicator.get_image_service = orig_img_service
...
class ReplicationCommandsTestCase(test_utils.BaseTestCase):
    ...
    @mock.patch.object(titicaca_replicator, 'lookup_command')
    def test_help(self, mock_lookup_command):
        option = mock.Mock()
        mock_lookup_command.return_value = "fake_return"

        titicaca_replicator.print_help(option, [])
        titicaca_replicator.print_help(option, ['dump'])
        titicaca_replicator.print_help(option, ['fake_command'])
    ...
    def test_replication_size(self):
        ...
        try:
            titicaca_replicator.get_image_service = get_image_service
            titicaca_replicator.replication_size(options, args)
            sys.stdout.seek(0)
    ...
    def test_replication_size_with_no_args(self):
        args = []
        command = titicaca_replicator.replication_size
    ...
    def test_human_readable_size(self):
        _human_readable_size = titicaca_replicator._human_readable_size
    ...
    def test_replication_dump(self):
        ...
        orig_img_service = titicaca_replicator.get_image_service
        self.addCleanup(setattr, titicaca_replicator,
                        'get_image_service', orig_img_service)
        titicaca_replicator.get_image_service = get_image_service
        titicaca_replicator.replication_dump(options, args)
    ...
    def test_replication_dump_with_no_args(self):
        args = []
        command = titicaca_replicator.replication_dump
        self.assertTrue(check_no_args(command, args))
    ...
    def test_replication_load(self):
        ...
        try:
            titicaca_replicator.get_image_service = get_image_service
            updated = titicaca_replicator.replication_load(options, args)
        finally:
            titicaca_replicator.get_image_service = orig_img_service
    ...
    def test_replication_load_with_no_args(self):
        args = []
        command = titicaca_replicator.replication_load
        self.assertTrue(check_no_args(command, args))
    ...
    def test_replication_livecopy(self):
        ...
        try:
            titicaca_replicator.get_image_service = get_image_service
            updated = titicaca_replicator.replication_livecopy(options, args)
        finally:
            titicaca_replicator.get_image_service = orig_img_service
    ...
    def test_replication_livecopy_with_no_args(self):
        args = []
        command = titicaca_replicator.replication_livecopy
        self.assertTrue(check_no_args(command, args))
    ...
    def test_replication_compare(self):
        ...
        try:
            titicaca_replicator.get_image_service = get_image_service
            differences = titicaca_replicator.replication_compare(options, args)
        finally:
            titicaca_replicator.get_image_service = orig_img_service
    ...
    def test_replication_compare_with_no_args(self):
        args = []
        command = titicaca_replicator.replication_compare
        self.assertTrue(check_no_args(command, args))
...
class ReplicationUtilitiesTestCase(test_utils.BaseTestCase):
    def test_check_upload_response_headers(self):
        titicaca_replicator._check_upload_response_headers({'status': 'active'},
                                                         None)

        d = {'image': {'status': 'active'}}
        titicaca_replicator._check_upload_response_headers({},
                                                         jsonutils.dumps(d))

        self.assertRaises(
            exception.UploadException,
            titicaca_replicator._check_upload_response_headers, {}, None)

    def test_image_present(self):
        client = FakeImageService(None, 'noauth')
        self.assertTrue(titicaca_replicator._image_present(
            client, '5dcddce0-cba5-4f18-9cf4-9853c7b207a6'))
        self.assertFalse(titicaca_replicator._image_present(
            client, uuid.uuid4()))
    ...
    def test_dict_diff(self):
        ...
        # matter here.
        self.assertFalse(titicaca_replicator._dict_diff(a, a))
        self.assertTrue(titicaca_replicator._dict_diff(a, b))
        self.assertTrue(titicaca_replicator._dict_diff(a, c))
        self.assertFalse(titicaca_replicator._dict_diff(a, d))
