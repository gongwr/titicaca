...
from titicaca.tests.unit import fixtures as titicaca_fixtures
...
class TestImageQuota(test_utils.BaseTestCase):
    ...
    def _get_image(self, location_count=1, image_size=10):
        ...
        image = titicaca.quota.ImageProxy(base_image, context, db_api, store)
        locations = []
    ...
    def test_quota_exceeded_no_size(self):
        ...
        with patch.object(titicaca.api.common, 'get_remaining_quota',
                          return_value=0):
...
class TestImagePropertyQuotas(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.image = titicaca.quota.ImageProxy(self.base_image,
                                             mock.Mock(),
                                             mock.Mock(),
                                             mock.Mock())
        ...
        self.image_repo_proxy = titicaca.quota.ImageRepoProxy(
...
class TestQuotaImageTagsProxy(test_utils.BaseTestCase):
    ...
    def test_add(self):
        proxy = titicaca.quota.QuotaImageTagsProxy(set([]))
        proxy.add('foo')
    ...
    def test_contains(self):
        proxy = titicaca.quota.QuotaImageTagsProxy(set(['foo']))
        self.assertIn('foo', proxy)
    ...
    def test_iter(self):
        ...
        proxy = titicaca.quota.QuotaImageTagsProxy(items.copy())
    ...
    def test_tags_attr_no_loop(self):
        proxy = titicaca.quota.QuotaImageTagsProxy(None)
        self.assertEqual(set([]), proxy.tags)

    def test_tags_deepcopy(self):
        proxy = titicaca.quota.QuotaImageTagsProxy(set(['a', 'b']))
...
class TestImageMemberQuotas(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.image_member_factory = titicaca.quota.ImageMemberFactoryProxy(
...
class TestImageKeystoneQuota(test_utils.BaseTestCase):
    def setUp(self):
        ...
        ksqf = titicaca_fixtures.KeystoneQuotaFixture(**default_limits)

        self.useFixture(ksqf)

        self.db_api = unit_test_utils.FakeDB()
        self.useFixture(fixtures.MockPatch('titicaca.quota.keystone.db',
                                           self.db_api))
