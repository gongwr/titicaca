...
class TestMetadefRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = titicaca.context.RequestContext(user=USER1,
                                                     tenant=TENANT1)
        self.namespace_repo = titicaca.db.MetadefNamespaceRepo(self.context,
                                                             self.db)
        self.property_repo = titicaca.db.MetadefPropertyRepo(self.context,
                                                           self.db)
        self.object_repo = titicaca.db.MetadefObjectRepo(self.context,
                                                       self.db)
        self.tag_repo = titicaca.db.MetadefTagRepo(self.context,
                                                 self.db)
        self.resource_type_repo = titicaca.db.MetadefResourceTypeRepo(
            self.context, self.db)
        self.namespace_factory = titicaca.domain.MetadefNamespaceFactory()
        self.property_factory = titicaca.domain.MetadefPropertyFactory()
        self.object_factory = titicaca.domain.MetadefObjectFactory()
        self.tag_factory = titicaca.domain.MetadefTagFactory()
        self.resource_type_factory = titicaca.domain.MetadefResourceTypeFactory()
