...
class TestTaskExecutor(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.executor = titicaca.async_.TaskExecutor(self.context,
                                                   self.task_repo,
                                                   self.image_repo,
                                                   self.image_factory)
    ...
    def test_with_admin_repo(self):
        ...
        executor = titicaca.async_.TaskExecutor(self.context,
                                              self.task_repo,
                                              self.image_repo,
                                              self.image_factory,
...
class TestImportTaskFlow(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.config(enabled_import_methods=[
            'titicaca-direct', 'web-download', 'copy-image'])
    ...
    def _get_flow(self, import_req=None):
        ...
        inputs['image_repo'].get.return_value = mock.MagicMock(
            extra_properties={'os_titicaca_import_task': mock.sentinel.task_id})
...
class TestSystemThreadPoolModel(test_utils.BaseTestCase):
    ...
    def test_base_model_spawn(self, mock_gte):
        ...
        model = titicaca.async_.ThreadPoolModel()
    ...
    def test_model_map(self):
        model = titicaca.async_.EventletThreadPoolModel()
    ...
    def test_base_model_init_with_size(self, mock_gte):
        ...
        with mock.patch.object(titicaca.async_, 'LOG') as mock_log:
            titicaca.async_.ThreadPoolModel(123)
    ...
    def test_set_threadpool_model_native(self):
        titicaca.async_.set_threadpool_model('native')
        self.assertEqual(titicaca.async_.NativeThreadPoolModel,
                         titicaca.async_._THREADPOOL_MODEL)

    def test_set_threadpool_model_eventlet(self):
        titicaca.async_.set_threadpool_model('eventlet')
        self.assertEqual(titicaca.async_.EventletThreadPoolModel,
                         titicaca.async_._THREADPOOL_MODEL)

    def test_set_threadpool_model_unknown(self):
        # Unknown threadpool models are not tolerated
        self.assertRaises(RuntimeError,
                          titicaca.async_.set_threadpool_model,
                          'danthread9000')
    ...
    def test_get_threadpool_model(self):
        titicaca.async_.set_threadpool_model('native')
        self.assertEqual(titicaca.async_.NativeThreadPoolModel,
                         titicaca.async_.get_threadpool_model())
