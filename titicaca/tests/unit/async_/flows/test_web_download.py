...
from titicaca.async_.flows._internal_plugins import web_download
from titicaca.async_.flows import api_image_import
import titicaca.common.exception
import titicaca.common.scripts.utils as script_utils
...
class TestWebDownloadTask(test_utils.BaseTestCase):
    ...
    def test_web_download_failed(self):
        ...
        with mock.patch.object(script_utils,
                               "get_image_data_iter") as mock_iter:
            mock_iter.side_effect = titicaca.common.exception.NotFound
            self.assertRaises(titicaca.common.exception.NotFound,
                              web_download_task.execute)
    ...
    @mock.patch.object(filesystem.Store, 'add')
    @mock.patch("titicaca.async_.flows._internal_plugins.web_download.store_api")
    def test_web_download_revert_with_failure(self, mock_store_api,
                                              mock_add):
        image = self.image_repo.get.return_value
        image.extra_properties['os_titicaca_importing_to_stores'] = 'foo'
        image.extra_properties['os_titicaca_failed_import'] = ''
        web_download_task = web_download._WebDownload(
            self.task.task_id, self.task_type, self.uri, self.action_wrapper,
            ['foo'])
        with mock.patch.object(script_utils,
        ...
        self.assertEqual(
            'foo', image.extra_properties['os_titicaca_importing_to_stores'])
        self.assertEqual(
            '', image.extra_properties['os_titicaca_failed_import'])
    ...
    def test_web_download_revert_with_failure_without_path(self,
        ...
        self.assertEqual(
            '', image.extra_properties['os_titicaca_importing_to_stores'])
        self.assertEqual(
            'foo', image.extra_properties['os_titicaca_failed_import'])
