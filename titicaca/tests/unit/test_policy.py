...
import titicaca.api.policy
from titicaca.common import exception
import titicaca.context
from titicaca.policies import base as base_policy
...
class ImageRepoStub(object):
    ...
    def get(self, *args, **kwargs):
        context = mock.Mock()
        policy = mock.Mock()
        return titicaca.api.policy.ImageProxy(
...
class ImageStub(object):
    ...
    def __init__(self, image_id=None, visibility='private',
        ...
        self.os_hash_value = hashlib.sha512(b'titicaca').hexdigest()
...
class TestPolicyEnforcer(base.IsolatedUnitTest):
    ...
    def test_policy_enforce_unregistered(self):
        ...
        context = titicaca.context.RequestContext(roles=[])

        self.assertRaises(titicaca.api.policy.policy.PolicyNotRegistered,
                          enforcer.enforce,
                          context, 'wibble', {})
    ...
    def test_policy_file_default_rules_default_location(self):
        ...
        context = titicaca.context.RequestContext(roles=['reader'])
    ...
    def _test_enforce_scope(self):
        ...
        context = titicaca.context.RequestContext(
    ...
    def test_ensure_context_object_is_passed_to_policy_enforcement(self):
        ...
        # Reset the mock and make sure titicaca.api.policy.Enforcer.check()
    ...
    def test_ensure_experimental_warning_is_logged_for_secure_rbac(self):
        ...
        with mock.patch.object(titicaca.api.policy, 'LOG') as mock_log:
            titicaca.api.policy.Enforcer(
                suppress_deprecation_warnings=True)
...
class TestPolicyEnforcerNoFile(base.IsolatedUnitTest):
    ...
    def test_policy_file_specified_but_not_found(self):
        ...
        admin_context = titicaca.context.RequestContext(roles=['admin'])
...
class TestImagePolicy(test_utils.BaseTestCase):
    ...
    def test_publicize_image_not_allowed(self):
        ...
        image = titicaca.api.policy.ImageProxy(
    ...
    def test_get_image_not_allowed(self):
        ...
        with mock.patch.object(titicaca.api.policy, 'ImageTarget') as target:
            target.return_value = image_target
            image_repo = titicaca.api.policy.ImageRepoProxy(
    ...
    def test_get_image_allowed(self):
        ...
        expected_target = dict(image.target)
        expected_target['project_id'] = image.owner
        self.assertIsInstance(image, titicaca.api.policy.ImageProxy)
        self.policy.enforce.assert_called_once_with(
            self.context, "get_image", expected_target)
    ...
    def test_new_image_visibility_public_not_allowed(self):
        self.policy.enforce.side_effect = exception.Forbidden
        image_factory = titicaca.api.policy.ImageFactoryProxy(
...
class TestMemberPolicy(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        image = titicaca.api.policy.ImageProxy(
            self.image_stub, self.context, self.policy)
        self.member_repo = titicaca.api.policy.ImageMemberRepoProxy(
...
class TestTaskPolicy(test_utils.BaseTestCase):
    ...
    def test_get_task_not_allowed(self):
        ...
        task_repo = titicaca.api.policy.TaskRepoProxy(
            self.task_repo_stub,
            {},
            self.policy
        )
    ...
    def test_get_task_allowed(self):
        ...
        self.assertIsInstance(task, titicaca.api.policy.TaskProxy)
        self.assertEqual('task_from_get', task.task)

    def test_get_tasks_not_allowed(self):
        rules = {"get_tasks": False}
        self.policy.set_rules(rules)
        task_repo = titicaca.api.policy.TaskStubRepoProxy(
            self.task_repo_stub,
            {},
            self.policy
        )
    ...
    def test_get_tasks_allowed(self):
        ...
        for i, task in enumerate(tasks):
            self.assertIsInstance(task, titicaca.api.policy.TaskStubProxy)
    ...
    def test_add_task_not_allowed(self):
        ...
        task = titicaca.api.policy.TaskProxy(self.task_stub, {}, self.policy)
...
class TestMetadefPolicy(test_utils.BaseTestCase):
    ...
    def test_md_namespace_not_allowed(self):
        ...
        self.policy.set_rules(rules)
        mdns_repo = titicaca.api.policy.MetadefNamespaceRepoProxy(
    ...
    def test_md_object_not_allowed(self):
        ...
        self.policy.set_rules(rules)
        mdobj_repo = titicaca.api.policy.MetadefObjectRepoProxy(
    ...
    def test_md_resource_type_not_allowed(self):
        ...
        self.policy.set_rules(rules)
        mdrt_repo = titicaca.api.policy.MetadefResourceTypeRepoProxy(
    ...
    def test_md_property_not_allowed(self):
        ...
        self.policy.set_rules(rules)
        mdprop_repo = titicaca.api.policy.MetadefPropertyRepoProxy(
    ...
    def test_md_tag_not_allowed(self):
        ...
        self.policy.set_rules(rules)
        mdtag_repo = titicaca.api.policy.MetadefTagRepoProxy(
...
class TestContextPolicyEnforcer(base.IsolatedUnitTest):
    ...
    def _do_test_policy_influence_context_admin(self,
        ...
        context = titicaca.context.RequestContext(roles=[context_role],
...
class TestImageTarget(base.IsolatedUnitTest):
    def test_image_target_ignores_locations(self):
        image = ImageStub()
        target = titicaca.api.policy.ImageTarget(image)
    ...
    def test_image_target_transforms(self):
        ...
        target = titicaca.api.policy.ImageTarget(fake_image)
