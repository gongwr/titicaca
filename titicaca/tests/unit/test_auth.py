...
from titicaca.tests.unit import utils as unittest_utils
...
class V2Token(object):
    ...
    def add_service_no_type(self):
        ...
        service_type = {"name": "titicaca_no_type"}
    ...
    def add_service(self, s_type, region_list=None):
        ...
        catalog = self.tok['access']['serviceCatalog']
        service_type = {"type": s_type, "name": "titicaca"}
        catalog.append(service_type)
        service = catalog[-1]
        endpoint_list = []
...
class TestImageMutability(utils.BaseTestCase):
    ...
    def _is_mutable(self, tenant, owner, is_admin=False):
        context = titicaca.context.RequestContext(tenant=tenant,
                                                is_admin=is_admin)
...
class TestImmutableImage(utils.BaseTestCase):
    def setUp(self):
        ...
        image_factory = titicaca.domain.ImageFactory()
    ...
    def test_get_data(self):
        ...
        image = titicaca.api.authorization.ImmutableImageProxy(
...
class TestImmutableTask(utils.BaseTestCase):
    def setUp(self):
        ...
        task_factory = titicaca.domain.TaskFactory()
...
class TestTaskFactoryProxy(utils.BaseTestCase):
    def setUp(self):
        ...
        self.context = titicaca.context.RequestContext(tenant=TENANT1)
        self.context_owner_is_none = titicaca.context.RequestContext()
