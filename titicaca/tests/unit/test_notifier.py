...
class ImageStub(titicaca.domain.Image):
...
class TaskStub(titicaca.domain.TaskStub):
    def run(self, executor):
        pass


class Task(titicaca.domain.Task):
...
class TestNotifier(utils.BaseTestCase):
    ...
    def test_set_defaults(self, mock_set_trans_defaults):
        ...
        mock_set_trans_defaults.assert_called_with('titicaca')
...
class TestImageNotifications(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = titicaca.context.RequestContext(tenant=TENANT2,
                                                     user=USER1)
        self.image_repo_stub = ImageRepoStub()
        self.notifier = unit_test_utils.FakeNotifier()
        self.image_repo_proxy = titicaca.notifier.ImageRepoProxy(
            self.image_repo_stub, self.context, self.notifier)
        self.image_proxy = titicaca.notifier.ImageProxy(
    ...
    def test_image_get(self):
        ...
        self.assertIsInstance(image, titicaca.notifier.ImageProxy)
        self.assertEqual('image_from_get', image.repo)

    def test_image_list(self):
        images = self.image_repo_proxy.list()
        self.assertIsInstance(images[0], titicaca.notifier.ImageProxy)
    ...
    def test_image_set_data_prepare_notification(self):
        ...
        def data_iterator():
            ...
            self.assertEqual([],
                             output_log['payload']['os_titicaca_failed_import'])
            yield 'abcd'
            yield 'efgh'
            insurance['called'] = True

        self.image_proxy.extra_properties[
            'os_titicaca_importing_to_stores'] = 'store1,store2'
        self.image_proxy.extra_properties['os_titicaca_failed_import'] = ''
    ...
    def test_image_set_data_upload_and_activate_notification(self):
        ...
        context = titicaca.context.RequestContext(tenant=TENANT2, user=USER1)
        fake_notifier = unit_test_utils.FakeNotifier()
        image_proxy = titicaca.notifier.ImageProxy(image, context, fake_notifier)
        ...
        def data_iterator():
            ...
            image_proxy.extra_properties[
                'os_titicaca_importing_to_stores'] = 'store2'

        image_proxy.extra_properties[
            'os_titicaca_importing_to_stores'] = 'store1,store2'
        image_proxy.extra_properties['os_titicaca_failed_import'] = ''
    ...
    def test_image_set_data_storage_full(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
            raise titicaca_store.StorageFull(message='Modern Major General')
    ...
    def test_image_set_data_storage_write_denied(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
            raise titicaca_store.StorageWriteDenied(message='The Very Model')
    ...
    def test_image_set_data_error(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
            raise exception.TiticacaException('Failed')

        self.assertRaises(exception.TiticacaException,
                          self.image_proxy.set_data, data_iterator(), 10)
...
class TestImageMemberNotifications(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.image = ImageStub(
            ...
            min_disk=10, checksum='ca425b88f047ce8ec45ee90e813ada91',
            locations=['http://127.0.0.1'])
        self.image_member = titicaca.domain.ImageMembership(
        ...
        self.image_member_repo_proxy = titicaca.notifier.ImageMemberRepoProxy(
            self.image_member_repo_stub, self.image,
            self.context, self.notifier)
        self.image_member_proxy = titicaca.notifier.ImageMemberProxy(
    ...
    def test_image_member_get(self):
        ...
        self.assertIsInstance(image_member, titicaca.notifier.ImageMemberProxy)
    ...
    def test_image_member_list(self):
        ...
        self.assertIsInstance(image_members[0],
                              titicaca.notifier.ImageMemberProxy)
...
class TestTaskNotifications(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = titicaca.context.RequestContext(
            tenant=TENANT2,
            user=USER1
        )
        self.task_repo_stub = TaskRepoStub()
        self.notifier = unit_test_utils.FakeNotifier()
        self.task_repo_proxy = titicaca.notifier.TaskRepoProxy(
            self.task_repo_stub,
            self.context,
            self.notifier
        )
        self.task_proxy = titicaca.notifier.TaskProxy(
            self.task,
            self.context,
            self.notifier
        )
        self.task_stub_proxy = titicaca.notifier.TaskStubProxy(
            self.task_stub,
            self.context,
            self.notifier
        )
    ...
    def test_task_run_notification(self):
        with mock.patch('titicaca.async_.TaskExecutor') as mock_executor:
