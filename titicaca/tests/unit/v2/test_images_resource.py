...
import titicaca.api.v2.image_actions
import titicaca.api.v2.images
...
import titicaca.schema
from titicaca.tests.unit import base
from titicaca.tests.unit.keymgr import fake as fake_keymgr
import titicaca.tests.unit.utils as unit_test_utils
from titicaca.tests.unit.v2 import test_tasks_resource
...
MULTIHASH1 = hashlib.sha512(b'titicaca').hexdigest()
...
def _domain_fixture(id, **kwargs):
    ...
    return titicaca.domain.Image(**properties)
...
class TestImagesController(base.IsolatedUnitTest):
    ...
    def setUp(self):
        ...
        self.controller = titicaca.api.v2.images.ImagesController(self.db,
                                                                self.policy,
                                                                self.notifier,
                                                                self.store)
        self.action_controller = (titicaca.api.v2.image_actions.
    ...
    @mock.patch('titicaca.db.simple.api.image_set_property_atomic')
    @mock.patch('titicaca.api.common.get_thread_pool')
    def test_image_import_raises_bad_request(self, mock_gpt, mock_spa):
        ...
        with mock.patch.object(
            ...
            # https://bugs.launchpad.net/titicaca/+bug/1712463 taskflow is not
    ...
    @mock.patch('titicaca.context.get_ksa_client')
    def test_image_import_proxies(self, mock_client):
        # Make sure that we proxy to the remote side when we need to
        self.config(
            worker_self_reference_url='http://titicaca-worker2.openstack.org')
        request = unit_test_utils.get_fake_request(
            '/v2/images/%s/import' % UUID4)
        with mock.patch.object(
                titicaca.notifier.ImageRepoProxy, 'get') as mock_get:
            mock_get.return_value = FakeImage(status='uploading')
            mock_get.return_value.extra_properties['os_titicaca_stage_host'] = (
                'https://titicaca-worker1.openstack.org')
            ...
            mock_client.return_value.post.assert_called_once_with(
                ('https://titicaca-worker1.openstack.org'
                 '/v2/images/%s/import') % UUID4,
                json={'method': {'name': 'titicaca-direct'}},
                timeout=60)
    ...
    @mock.patch('titicaca.context.get_ksa_client')
    @mock.patch.object(titicaca.notifier.ImageRepoProxy, 'get')
    @mock.patch.object(titicaca.notifier.ImageRepoProxy, 'remove')
    def test_image_delete_deletes_locally_on_error(self, mock_remove, mock_get,
        ...
        image.extra_properties['os_titicaca_stage_host'] = (
            'https://titicaca-worker1.openstack.org')
    ...
    def test_update_reserved_not_counted_in_quota(self):
        ...
        self.db.image_update(None, UUID1, {'properties': {
            'os_titicaca_foo': '123', 'os_titicaca_bar': 456}})
    ...
    def test_prop_protection_with_create_and_permitted_role(self):
        enforcer = titicaca.api.policy.Enforcer(
    ...
    @mock.patch.object(titicaca.quota, '_calc_required_size')
    @mock.patch.object(titicaca.location, '_check_image_location')
    @mock.patch.object(titicaca.location.ImageRepoProxy, '_set_acls')
    ...
    def test_verify_staging_data_deleted_on_image_delete(
        ...
        try:
            ...
            mock_store.assert_called_once_with("os_titicaca_staging_store")
    ...
    @mock.patch('titicaca.db.simple.api.image_set_property_atomic')
    @mock.patch.object(titicaca.notifier.TaskFactoryProxy, 'new_task')
    @mock.patch.object(titicaca.domain.TaskExecutorFactory, 'new_task_executor')
    @mock.patch('titicaca.api.common.get_thread_pool')
    @mock.patch('titicaca.quota.keystone.enforce_image_size_total')
    def test_image_import(self, mock_enforce, mock_gtp, mock_nte, mock_nt,
        ...
        with mock.patch.object(
            ...
            output = self.controller.import_image(
                request, UUID4, {'method': {'name': 'titicaca-direct'}})
        ...
        mock_spa.assert_called_once_with(UUID4, 'os_titicaca_import_task',
    ...
    @mock.patch.object(titicaca.domain.TaskFactory, 'new_task')
    @mock.patch.object(titicaca.notifier.ImageRepoProxy, 'get')
    ...
    @mock.patch('titicaca.db.simple.api.image_set_property_atomic')
    @mock.patch('titicaca.context.RequestContext.elevated')
    @mock.patch.object(titicaca.domain.TaskFactory, 'new_task')
    @mock.patch.object(titicaca.notifier.ImageRepoProxy, 'get')
    ...
    def test_image_import_locked(self, mock_get):
        ...
        image.extra_properties['os_titicaca_import_task'] = task['id']
        mock_get.return_value = image

        request = unit_test_utils.get_fake_request(tenant=TENANT1)
        req_body = {'method': {'name': 'titicaca-direct'}}
    ...
    @mock.patch('titicaca.db.simple.api.image_set_property_atomic')
    @mock.patch('titicaca.db.simple.api.image_delete_property_atomic')
    @mock.patch.object(titicaca.notifier.TaskFactoryProxy, 'new_task')
    @mock.patch.object(titicaca.notifier.ImageRepoProxy, 'get')
    def test_image_import_locked_by_reaped_task(self, mock_get, mock_nt,
        ...
        image.extra_properties['os_titicaca_import_task'] = 'missing'
        mock_get.return_value = image
        ...
        mock_dpi.assert_called_once_with(image.id, 'os_titicaca_import_task',
                                         'missing')
        # We should have atomically grabbed the lock with our task id
        mock_spi.assert_called_once_with(image.id, 'os_titicaca_import_task',
                                         'mytask')

    @mock.patch.object(titicaca.notifier.ImageRepoProxy, 'save')
    @mock.patch('titicaca.db.simple.api.image_set_property_atomic')
    @mock.patch('titicaca.db.simple.api.image_delete_property_atomic')
    @mock.patch.object(titicaca.notifier.TaskFactoryProxy, 'new_task')
    @mock.patch.object(titicaca.notifier.ImageRepoProxy, 'get')
    def test_image_import_locked_by_bustable_task(self, mock_get, mock_nt,
        ...
        image.extra_properties['os_titicaca_import_task'] = task['id']
        image.extra_properties['os_titicaca_importing_to_stores'] = 'store2'
        mock_get.return_value = image
        ...
        if task_status == 'processing':
            self.assertNotIn('store2',
                             image.extra_properties[
                                 'os_titicaca_importing_to_stores'])
    ...
    def test_cleanup_stale_task_progress(self):
        ...
        image.extra_properties = {'os_titicaca_failed_import': 'store3'}
        self.controller._cleanup_stale_task_progress(img_repo, image, task)
        img_repo.save.assert_not_called()
        ...
        image.extra_properties = {
            'os_titicaca_importing_to_stores': 'foo,store1,bar',
            'os_titicaca_failed_import': 'foo,store2,bar',
        }
        self.controller._cleanup_stale_task_progress(img_repo, image, task)
        img_repo.save.assert_called_once_with(image)
        self.assertEqual({'os_titicaca_importing_to_stores': 'foo,bar',
                          'os_titicaca_failed_import': 'foo,bar'},
                         image.extra_properties)
    ...
    def test_enforce_lock_log_not_bustable(self, task_status='processing'):
        ...
        with mock.patch.object(titicaca.api.v2.images, 'LOG') as mock_log:
            self.assertRaises(exception.Conflict,
...
class TestImagesDeserializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.deserializer = titicaca.api.v2.images.RequestDeserializer()
    ...
    def test_update_reserved_attributes(self):
        samples = {
            'deleted': False,
            'deleted_at': ISOTIME,
            'os_titicaca_import_task': 'foo',
            'os_titicaca_anything': 'bar',
            'os_titicaca_': 'baz',
            'os_titicaca': 'bat',
        }
    ...
    KNOWN_IMPORT_METHODS = ['titicaca-direct', 'web-download']
...
class TestImagesDeserializerWithExtendedSchema(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        schema = titicaca.api.v2.images.get_schema(custom_image_properties)
        self.deserializer = titicaca.api.v2.images.RequestDeserializer(schema)
...
class TestImagesSerializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.serializer = titicaca.api.v2.images.ResponseSerializer()
        self.fixtures = [
    ...
    def test_image_stage_host_hidden(self):
        # Make sure that os_titicaca_stage_host is not exposed to clients
        response = webob.Response()
        self.serializer.show(response,
                             mock.MagicMock(extra_properties={
                                 'foo': 'bar',
                                 'os_titicaca_stage_host': 'http://foo'}))
        actual = jsonutils.loads(response.body)
        self.assertIn('foo', actual)
        self.assertNotIn('os_titicaca_stage_host', actual)
...
class TestImagesSerializerWithExtendedSchema(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        schema = titicaca.api.v2.images.get_schema(custom_image_properties)
        self.serializer = titicaca.api.v2.images.ResponseSerializer(schema)
...
class TestImagesSerializerWithAdditionalProperties(test_utils.BaseTestCase):
    ...
    def test_show(self):
        serializer = titicaca.api.v2.images.ResponseSerializer()
        expected = {
            'id': UUID2,
            'name': 'image-2',
            'status': 'queued',
...
class TestImageSchemaFormatConfiguration(test_utils.BaseTestCase):
    ...
    def test_default_disk_formats(self):
        schema = titicaca.api.v2.images.get_schema()
...
class TestProxyHelpers(base.IsolatedUnitTest):
    def test_proxy_response_error(self):
        e = titicaca.api.v2.images.proxy_response_error(123, 'Foo')
    ...
    def test_is_proxyable(self):
        controller = titicaca.api.v2.images.ImagesController(None, None,
                                                           None, None)
        ...
        mock_image.extra_properties['os_titicaca_stage_host'] = 'http://worker1'
        self.assertFalse(controller.is_proxyable(mock_image))

        mock_image.extra_properties['os_titicaca_stage_host'] = 'http://worker2'
