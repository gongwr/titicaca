...
def _domain_fixture(task_id, **kwargs):
    ...
    task = titicaca.domain.Task(**task_properties)
    return task


CONF = cfg.CONF
CONF.import_opt('task_time_to_live', 'titicaca.common.config', group='task')
...
class TestTasksController(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self._create_tasks()
        self.controller = titicaca.api.v2.tasks.TasksController(self.db,
                                                              self.policy,
                                                              self.notifier,
                                                              self.store)
        self.gateway = titicaca.gateway.Gateway(self.db, self.store,
    ...
    @mock.patch.object(titicaca.gateway.Gateway, 'get_task_factory')
    @mock.patch.object(titicaca.gateway.Gateway, 'get_task_executor_factory')
    @mock.patch.object(titicaca.gateway.Gateway, 'get_task_repo')
    ...
    @mock.patch('titicaca.common.scripts.utils.get_image_data_iter')
    @mock.patch('titicaca.common.scripts.utils.validate_location_uri')
...
class TestTasksDeserializerPolicies(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.policy = unit_test_utils.FakePolicyEnforcer()
        self.deserializer = titicaca.api.v2.tasks.RequestDeserializer(
...
class TestTasksSerializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.serializer = titicaca.api.v2.tasks.ResponseSerializer()
        self.fixtures = [
