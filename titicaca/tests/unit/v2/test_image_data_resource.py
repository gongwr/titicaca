...
from titicaca_store._drivers import filesystem
from oslo_config import cfg
import webob

import titicaca.api.policy
import titicaca.api.v2.image_data
...
class TestImagesController(base.StoreClearingUnitTest):
    ...
    def setUp(self):
        ...
        self.controller = titicaca.api.v2.image_data.ImageDataController()
        ...
        # some realistic data
        patcher = mock.patch('titicaca.api.v2.policy.check_is_image_mutable')
        patcher.start()
    ...
    @mock.patch.object(titicaca.api.policy.Enforcer, 'enforce')
    ...
    @mock.patch('titicaca.common.trust_auth.TokenRefresher')
    ...
    def test_upload_storage_full(self):
        request = unit_test_utils.get_fake_request()
        image = FakeImage()
        image.set_data = Raise(titicaca_store.StorageFull)
        self.image_repo.result = image
    ...
    def test_upload_storage_write_denied(self):
        ...
        image = FakeImage()
        image.set_data = Raise(titicaca_store.StorageWriteDenied)
        self.image_repo.result = image
    ...
    def test_upload_storage_store_disabled(self):
        ...
        request = unit_test_utils.get_fake_request(user=unit_test_utils.USER3)
        image = FakeImage()
        image.set_data = Raise(titicaca_store.StoreAddDisabled)
        self.image_repo.result = image
    ...
    def test_restore_image_when_staging_failed(self, mock_store_add):
        mock_store_add.side_effect = titicaca_store.StorageWriteDenied()
        request = unit_test_utils.get_fake_request()
    ...
    def test_image_already_on_staging(self):
        ...
        with mock.patch.object(filesystem.Store, 'add') as mock_store_add:
            ...
            self.controller.stage(request, image_id, 'YYYY', 4)
            self.assertEqual('uploading', image.status)
            mock_store_add.side_effect = titicaca_store.Duplicate()
            self.assertEqual(4, image.size)
            self.assertRaises(webob.exc.HTTPConflict, self.controller.stage,
                              request, image_id, 'YYYY', 4)

    @mock.patch.object(titicaca_store.driver.Store, 'configure')
    ...
    def test_image_stage_raises_storage_full(self, mock_store_add):
        mock_store_add.side_effect = titicaca_store.StorageFull()
        image_id = str(uuid.uuid4())
        request = unit_test_utils.get_fake_request()
        image = FakeImage(image_id=image_id)
        self.image_repo.result = image
    ...
    def _test_image_stage_records_host(self, expected_url):
        ...
        if expected_url is None:
            self.assertNotIn('os_titicaca_stage_host', image.extra_properties)
        else:
    ...
    def test_image_stage_fail_does_not_set_host(self):
        ...
        image = FakeImage(image_id=image_id)
        self.image_repo.result = image
        exc_cls = titicaca_store.exceptions.StorageFull
...
class TestImageDataDeserializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.deserializer = titicaca.api.v2.image_data.RequestDeserializer()
    ...
    def test_stage_without_titicaca_direct(self):
...
class TestImageDataSerializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.serializer = titicaca.api.v2.image_data.ResponseSerializer()
    ...
    def test_download_failure_with_valid_range(self):
        with mock.patch.object(titicaca.api.policy.ImageProxy,
                               'get_data') as mock_get_data:
            mock_get_data.side_effect = titicaca_store.NotFound(image="image")
        request = wsgi.Request.blank('/')
        request.environ = {}
    ...
    def test_download_forbidden(self):
        ...
        self.mock_object(titicaca.api.policy.ImageProxy,
                         'get_data',
                         get_data)
        request = wsgi.Request.blank('/')
        request.environ = {}
    ...
    def test_download_service_unavailable(self):
        ...
        with mock.patch.object(titicaca.api.policy.ImageProxy,
                               'get_data') as mock_get_data:
            mock_get_data.side_effect = titicaca_store.RemoteServiceUnavailable()
    ...
    def test_download_store_get_not_support(self):
        ...
        with mock.patch.object(titicaca.api.policy.ImageProxy,
                               'get_data') as mock_get_data:
            mock_get_data.side_effect = titicaca_store.StoreGetNotSupported()
    ...
    def test_download_store_random_get_not_support(self):
        ...
        with mock.patch.object(titicaca.api.policy.ImageProxy,
            ...
            err = titicaca_store.StoreRandomGetNotSupported(offset=0,
                                                          chunk_size=0)
