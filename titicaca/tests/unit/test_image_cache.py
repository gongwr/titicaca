...
from titicaca import async_
from titicaca.common import exception
from titicaca import context
from titicaca import gateway as titicaca_gateway
from titicaca import image_cache
...
from titicaca.tests.utils import xattr_writes_supported
...
class ImageCacheTestCase(object):
    ...
    def test_caching_iterator_handles_backend_failure(self):
        ...
        def faulty_backend():
            ...
            for d in data:
                if d == b'Fail':
                    raise exception.TiticacaException('Backend failure')
                yield d
        ...
        image_id = '1'
        self.assertRaises(exception.TiticacaException, consume, image_id)
    ...
    def test_gate_caching_iter_bad_checksum(self):
        ...
        def reader():
            for chunk in img_iter:
                pass
        self.assertRaises(exception.TiticacaException, reader)
...
class TestImageCacheSqlite(test_utils.BaseTestCase,
    ...
    @mock.patch('titicaca.db.get_api')
    def _test_prefetcher(self, mock_get_db):
        ...
        gateway = titicaca_gateway.Gateway()
