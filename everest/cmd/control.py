...
"""
Helper script for starting/stopping/reloading Everest server programs.
...
possible_topdir = os.path.normpath(os.path.join(os.path.abspath(sys.argv[0]),
                                   os.pardir,
                                   os.pardir))
if os.path.exists(os.path.join(possible_topdir, 'everest', '__init__.py')):
...
RELOAD_SERVERS = ['everest-api']
GRACEFUL_SHUTDOWN_SERVERS = ['everest-api', 'everest-scrubber']
...
def pid_files(server, pid_file):
    ...
    if pid_file:
        ...
        if os.path.exists('/var/run/everest/%s.pid' % server):
            pid_files = ['/var/run/everest/%s.pid' % server]
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(servers=['everest-' + s for s in ALL_SERVERS])
