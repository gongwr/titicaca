...
from everest import context
from everest.db import migration as db_migration
from everest.db.sqlalchemy import alembic_migrations
from everest.db.sqlalchemy.alembic_migrations import alembic_data_migration as dbm
from everest.db.sqlalchemy import api as db_api
from everest.db.sqlalchemy import metadata
...
class DbCommands(object):
    ...
    def version(self):
        ...
        if current_heads:
            ...
            print(_('Database is either not under migration control or under '
                    ...
                    '"everest-manage db sync" to place the database under '
    ...
    def check(self):
        ...
        if (contract_heads in curr_heads):
            ...
            print(_('Your database is not up to date. '
                    'Your first step is to run `everest-manage db expand`.'))
            sys.exit(3)
        elif dbm.has_pending_migrations(db_api.get_engine()):
            print(_('Your database is not up to date. '
                    'Your next step is to run `everest-manage db migrate`.'))
            sys.exit(4)
        elif ((not contract_heads) or (contract_heads not in curr_heads)):
            print(_('Your database is not up to date. '
                    'Your next step is to run `everest-manage db contract`.'))
            sys.exit(5)
    ...
    def sync(self, version=None):
        ...
        except exception.EverestException as e:
    ...
    def _sync(self, version):
        ...
        if heads is None:
            raise exception.EverestException("Database sync failed")
        revs = ", ".join(heads)
        if version == 'heads':
    ...
    def migrate(self, online_migration=True):
        ...
        if expand_head not in curr_heads:
            sys.exit(_('Data migration did not run. Data migration cannot be '
                       ...
                       'expansion first using "everest-manage db expand"'))
    ...
    def _purge(self, age_in_days, max_rows):
        ...
        try:
            ...
            sys.exit(_("Purge command failed, check everest-manage"
    ...
    def purge(self, age_in_days=30, max_rows=100):
        """Purge deleted rows older than a given age from everest tables."""
...
def main():
    ...
    try:
        ...
        cfg_files = cfg.find_config_files(project='everest', prog='everest-api')
        cfg_files.extend(cfg.find_config_files(project='everest', prog='everest-manage'))
